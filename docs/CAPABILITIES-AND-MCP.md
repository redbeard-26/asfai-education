...
The **ASFAI Learning** plugin installs the one-tool private storage companion beside the public eight-tool MCP. It turns storage instructions into actual atomic local JSON or authenticated Solid operations and owner-controlled classroom signatures. The only additional tool is `asfai_personal_storage`; specialized guidance remains on demand. See [Personal storage MCP companion](PERSONAL-STORAGE-COMPANION.md).
